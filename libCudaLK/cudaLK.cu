...
// - use "magic kernel" to compute the gradient
// - adaptative gain  ?
...
__global__ void convertGreyToFloat(unsigned char *d_in, float *d_out, int N)
{
  int idx = blockIdx.x*blockDim.x + threadIdx.x;
...
__global__ void kernelScharrX(const float *in, int _w, int _h, float *out) {
  // Pattern : // Indexes :
  // -3 -10 -3 // a1 b1 c1
  //  0   0  0 // a2 b2 c2
  //  3  10  3 // a3 b3 c3
...
// Compute spatial derivatives using Scharr operator - Naive implementation..
__global__ void kernelScharrY(const float *in, int _w, int _h, float *out) {
  int x = blockIdx.x*blockDim.x + threadIdx.x;
  int y = blockIdx.y*blockDim.y + threadIdx.y;
...
__global__ void kernelSobelX(const float *in, int _w, int _h, float *out) {
  // Pattern : // Indexes :
  // -1 -2 -1 // a1 b1 c1
  //  0  0  0 // a2 b2 c2
  //  1  2  1 // a3 b3 c3
...
// Compute spatial derivatives using Scharr operator - Naive implementation..
__global__ void kernelSobelY(const float *in, int _w, int _h, float *out) {
  int x = blockIdx.x*blockDim.x + threadIdx.x;
  int y = blockIdx.y*blockDim.y + threadIdx.y;
...
__global__ void kernelAdd(const float *in1,
                          const float *in2,
                          int _w,
                          int _h,
                          float *out) {
...
// Low pass gaussian-like filtering before subsampling
__global__ void kernelSmoothY(float *in, int w, int h, float *out)
{
  int x = blockIdx.x*blockDim.x + threadIdx.x;
  int y = blockIdx.y*blockDim.y + threadIdx.y;
...
__global__ void compute_spatial_grad(float  *coord_gpu,
                                     char   *status_gpu,
...
__global__ void track_pts_slim(float *coord_gpu,
                               float *dx_gpu,
                               float *dy_gpu,
                               char  *status_gpu,
                               const float *gpu_neighbourhood_det,
                               const float *gpu_neighbourhood_Iyy,
                               const float *gpu_neighbourhood_Ixy,
                               const float *gpu_neighbourhood_Ixx)
{
  int idx = (blockIdx.x*blockDim.x + threadIdx.x)
            + (blockIdx.y*blockDim.y + threadIdx.y) * gridDim.x * blockDim.x; // "2D" indexing
...
// Kernel to compute the tracking
__global__ void track_pts_weighted(float *coord_gpu,
                                   float *dx_gpu,
                                   float *dy_gpu,
                                   char  *status_gpu,
                                   bool  rectified)
...
void cudaLK::computeDerivatives(const float *in,
                                float       *deriv_buff_x,
                                float       *deriv_buff_y,
                                int         pyr_level,
...
void cudaLK::initMem4Frame()
{
  ...
  printf("CUDA : 4Frames memory allocated\n");
}
